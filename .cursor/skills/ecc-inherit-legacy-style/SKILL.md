...
- Read/Write access to the project root (generates `.ai-style-rules.md` and optionally `project configuration`)
...
| Option | Mechanism |
|---|---|
| **1** Soft hook (recommended) | Write `@.ai-style-rules.md` reference into project `project configuration` |
...
When `.ai-style-rules.md` is in context (loaded via project configuration), every code-writing task must open with a **compliance declaration** in the reasoning chain, naming the exemplar being followed and the DONTs being avoided.
...
- **Per-Turn Enforcement** — When hooked via `project configuration`, every code-writing task opens with a compliance declaration naming the exemplar followed and the DONTs avoided.

## Output Specification

- `.ai-style-rules.md` at project root (with commit fingerprint + scale tier in header)
- Optionally `project configuration` with `@.ai-style-rules.md` reference
...
- The project configuration soft hook (`@.ai-style-rules.md`) is usually sufficient; hard hook only if the user wants mechanical enforcement

## Related Skills

- `init` — initialize a new project configuration with codebase documentation
...
3. **Enforcing DONTs via project configuration**
