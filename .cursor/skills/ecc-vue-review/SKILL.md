...
For pure `.ts`/`.js` changes with no Vue imports, use `/ecc-code-review` command (general) or invoke `typescript-reviewer` directly.
...
- Use `/ecc-code-review` command for non-Vue-specific concerns on the same PR
