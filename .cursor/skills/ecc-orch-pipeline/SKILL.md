...
> `/feature-dev`, `/ecc-plan` command, `/ecc-code-review` command, `/ecc-build-fix` command, `/ecc-refactor-clean` command, and
...
- **2. Plan** — delegate to the `ecc-planner` skill (or `architect` /
...
- **4. Implement (TDD)** — drive each task through the `ecc-tdd-guide` skill (or the `tdd-workflow` skill):
  red → green → refactor. Honor the operation's first-move rule.
- **5. Review** — `ecc-code-reviewer` skill / `/ecc-code-review` command. Add `security-reviewer`
...
| Implement | `tdd-guide` (or `tdd-workflow` skill) | `build-error-resolver` / `/ecc-build-fix` command on build breaks |
| Review | `code-reviewer` / `/ecc-code-review` command | language reviewer (`python-reviewer`, `typescript-reviewer`, …) |
| Security | `security-reviewer` | — |
| MVP inner loop | `/gan-build "<brief>" --skip-planner` | drives `gan-generator` → `gan-evaluator`; tune `--max-iterations` / `--pass-threshold` |

Match the language reviewer to the repo (see the repo's own `project configuration`).
