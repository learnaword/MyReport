---
name: ecc-plan
description: Implementation planning for complex features and architectural changes. Use when starting new features, complex refactoring, or when requirements are unclear. Restates requirements, assesses risks, creates step-by-step plans, and waits for confirmation.
---
...
Run inline by default. Do not call the Task tool or any subagent by default. This keeps `/ecc-plan` command usable from plugin installs that ship commands without agent files.
...
```
User: /ecc-plan I need to add real-time notifications when markets resolve
...
- Use `/ecc-build-fix` command if build errors occur
- Use `/ecc-code-review` command to review completed implementation
...
ECC also provides a `ecc-planner` skill for manual installs that include agent files. Use it only when the local runtime already exposes that subagent and the user explicitly asks you to delegate planning.
