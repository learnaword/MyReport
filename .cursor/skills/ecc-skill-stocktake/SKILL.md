...
| Path | Description |
|------|-------------|
| `~/.cursor/skills/` | Global skills (all projects) |
...
**Results cache:** `~/.cursor/skills/ecc-skill-stocktake/results.json`

## Quick Scan Flow

Re-evaluate only skills that have changed since the last run (5–10 min).

1. Read `~/.cursor/skills/ecc-skill-stocktake/results.json`
2. Run: `bash ~/.cursor/skills/ecc-skill-stocktake/scripts/quick-diff.sh \
         ~/.cursor/skills/ecc-skill-stocktake/results.json`
   (Project dir is auto-detected from `$PWD/.claude/skills`; pass it explicitly only if needed)
3. If output is `[]`: report "No changes since last run." and stop
4. Re-evaluate only those changed files using the same Phase 2 criteria
5. Carry forward unchanged skills from previous results
6. Output only the diff
7. Run: `bash ~/.cursor/skills/ecc-skill-stocktake/scripts/save-results.sh \
         ~/.cursor/skills/ecc-skill-stocktake/results.json <<< "$EVAL_RESULTS"`

## Full Stocktake Flow

### Phase 1 — Inventory

Run: `bash ~/.cursor/skills/ecc-skill-stocktake/scripts/scan.sh`
...
- [ ] Overlap with MEMORY.md / project configuration checked
...
- **Uniqueness**: value not replaceable by MEMORY.md / project configuration / another skill
...
```json
{
  ...
  "skills": {
    "skill-name": {
      "path": "~/.cursor/skills/ecc-skill-name/SKILL.md",
      "verdict": "Keep",
