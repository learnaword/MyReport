...
Safely open-source any project through a 3-stage pipeline: **Fork** (strip secrets) → **Sanitize** (verify clean) → **Package** (project configuration + setup.sh + README).
...
| `/opensource package PROJECT` | Generate project configuration + setup.sh + README |
...
Spawn the `ecc-opensource-forker` skill:
...
Spawn the `ecc-opensource-sanitizer` skill:
...
Spawn the `ecc-opensource-packager` skill:
...
Generate:
1. project configuration (commands, architecture, key files)
...
Show each project with pipeline progress (FORK_REPORT.md, SANITIZATION_REPORT.md, project configuration presence).
...
$HOME/opensource-staging/
  my-project/
    ...
    project configuration                # From packager agent
