...
`~/.cursor/projects/**` and reports stale `ScheduleWakeup` calls or `Bash`
...
- `ecc loop-status --watch --write-dir ~/.cursor/loops` maintains
...
timeout Cursor runtime tool calls.
