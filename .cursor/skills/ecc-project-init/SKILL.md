...
1. Default to dry-run. Do not modify `project configuration`, settings files, rules, skills, or install state until the user approves the concrete plan.
2. Preserve existing project guidance. If `project configuration`, `.claude/settings.local.json`, `.cursor/`, `.codex/`, `.gemini/`, `.opencode/`, `.codebuddy/`, `.joycode/`, or `.qwen/` already exists, inspect it and propose a merge/append plan instead of overwriting.
...
If the user wants a `project configuration` starter, generate it separately from the installer plan and keep it minimal:
...
Never replace an existing `project configuration` without showing a diff and receiving approval.
