...
ECC-native around the `/ecc-plan` command confirmation gate, with zero dependencies.

## When to Use

- You just wrote a plan artifact (`.claude/plans/*.plan.md` from `/ecc-plan` command) and
...
Do NOT use for: code review of diffs (`/ecc-code-review` command), running web apps, or
...
(Cursor, Codex, Cursor, Gemini, OpenCode, Copilot). Trigger it however your
harness surfaces skills — e.g. `/plan-canvas` in Cursor, `$plan-canvas` in
...
- The server is loopback-only and exits after 30 idle minutes
  ...
  in `~/.cursor/plan-canvas/` (`ECC_PLAN_CANVAS_STATE_DIR`).

## Examples

**Plan approval flow** — `/ecc-plan` command writes
