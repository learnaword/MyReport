...
| Skills directory | `ls ~/.cursor/skills ~/.codex/skills` where applicable | Say no local skill catalog was available |
...
2. Is there an MCP for this? → Check `~/.cursor/settings.json` and search
3. Is there a skill for this? → Check `~/.cursor/skills/`
...
Agent(subagent_type="general-purpose", prompt="
  ...
  Search: npm/PyPI, MCP servers, Cursor skills, GitHub
  Return: Structured comparison with recommendation
")
```

Older Cursor docs may call this `Task(...)`; use the current agent/subagent
