...
Treat Cursor as a persistent runtime / operating system rather than a chat session. This skill codifies the architecture used by production agentic setups: a kernel config that routes tasks to specialist agents, persistent file-based memory, scheduled automation, and a JSON/markdown data layer.

## When to Activate

- Building a multi-agent workflow inside Cursor
- Setting up persistent Cursor automation that survives session restarts
...
```
project-root/
├── project configuration          # Kernel: identity, routing rules, agent registry
...
|---|---|---|
| Kernel (`project configuration`) | Identity, routing, model policies, agent registry | Git-tracked |
...
`project configuration` is the kernel. It acts as the COO / orchestrator. Claude reads it at session start and uses it to route work.

### Kernel Structure

```markdown
# project configuration - Agentic OS Kernel
...
For parallel execution, use Cursor's background task capability or shell scripts that invoke Cursor with specific agent contexts.
...
Place command files in `.claude/commands/<command-name>.md`. Cursor auto-discovers them. Users invoke them with `/<command-name>`.
...
Agentic OS tasks run on a schedule using external cron, not Cursor's built-in cron (which dies when the session ends).
...
Starting fresh every time Cursor opens.
```
...
```markdown
# BAD - API keys in agent files or project configuration
...
Keep routing declarative in `project configuration` markdown tables. It is inspectable, editable, and debuggable.

## Best Practices

- [ ] `project configuration` is under 200 lines and fits in context window
...
- [ ] Scheduled tasks use external cron (LaunchAgent, systemd, pm2), not Cursor's session cron
- [ ] Cost tracking: log API spend per session in `data/logs/<date>-costs.json`
- [ ] One project = one Agentic OS. Do not share a single `project configuration` across unrelated projects.
