...
Scripts live at: `~/.cursor/skills/ecc-ck/commands/` (expand `~` with `$HOME`).
...
The hook at `~/.cursor/skills/ecc-ck/hooks/session-start.mjs` must be registered in
`~/.cursor/settings.json` to auto-load project context on session start:

```json
{
  "hooks": {
    "SessionStart": [
      { "hooks": [{ "type": "command", "command": "node \"~/.cursor/skills/ecc-ck/hooks/session-start.mjs\"" }] }
    ]
  }
}
```
...
unsaved sessions, git activity since last save, and goal mismatches vs project configuration.
