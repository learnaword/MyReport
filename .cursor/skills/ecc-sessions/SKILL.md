...
Manage Cursor session history - list, load, alias, and edit sessions stored in `~/.cursor/session-data/` with legacy reads from `~/.cursor/sessions/`.
...
- Sessions are stored as markdown files in `~/.cursor/session-data/` with legacy reads from `~/.cursor/sessions/`
- Aliases are stored in `~/.cursor/session-aliases.json`
