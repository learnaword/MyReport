...
Produces a **Product Requirements Document** — the requirements-phase artifact of the SDLC. Captures *what* must be true for success and *why*, and stops before *how*. Implementation decomposition is delegated to `/ecc-plan` command.
...
| Write `.claude/prds/{name}.prd.md` | Produce an implementation plan — that's `/ecc-plan` command |

If you find yourself writing implementation detail, stop and cut it. It belongs in `/ecc-plan` command.
...
## Delivery Milestones
<!-- Business outcomes, not engineering tasks. /ecc-plan turns each into a plan. -->
...
Next step: /ecc-plan .claude/prds/{name}.prd.md
  → /ecc-plan will pick the next pending milestone and produce an implementation plan.
```

## Integration

- `/ecc-plan <prd-path>` — consume the PRD and produce an implementation plan for the next pending milestone.
...
- **NO_IMPLEMENTATION_DETAIL**: file paths, libraries, or task breakdowns are absent — if they appeared, move them to the `/ecc-plan` command step.
