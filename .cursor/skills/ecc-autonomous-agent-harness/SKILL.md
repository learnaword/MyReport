...
Turn Cursor into a persistent, self-directing agent system using only native features and MCP servers.
...
Use Cursor's built-in memory system enhanced with MCP memory server for structured data.

**Built-in memory** (`~/.cursor/projects/*/memory/`):
...
Write to ~/.cursor/projects/*/memory/ for cross-session recall
...
Use Cursor's scheduled tasks to create recurring agent operations.
...
Trigger Cursor agents remotely for event-driven workflows.
...
Write task queue to ~/.cursor/projects/*/memory/task-queue.md
...
|------------------|---------------|-----|
| Gateway/Router | Cursor dispatch + crons | Scheduled tasks trigger agent sessions |
