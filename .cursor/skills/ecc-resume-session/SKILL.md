...
```
/resume-session                                                      # loads most recent file in ~/.cursor/session-data/
/resume-session 2024-01-15                                           # loads most recent session for that date
/resume-session ~/.cursor/session-data/2024-01-15-abc123de-session.tmp  # loads a current short-id session file
/resume-session ~/.cursor/sessions/2024-01-15-session.tmp               # loads a specific legacy-format file
```
...
- If it looks like a date (`YYYY-MM-DD`), search `~/.cursor/session-data/` first, then the legacy
  `~/.cursor/sessions/`, for files matching `YYYY-MM-DD-session.tmp` (legacy format) or
