...
For pure `.ts`/`.js` changes with no React imports, use `/ecc-code-review` command (general) or invoke `typescript-reviewer` directly.

## Scope vs `/ecc-code-review` command and TypeScript Review
...
| `/ecc-code-review` command | Generic uncommitted-changes or PR review |
...
- Use `/ecc-code-review` command for non-React-specific concerns on the same PR
