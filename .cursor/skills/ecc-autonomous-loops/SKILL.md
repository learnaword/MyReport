...
Patterns, architectures, and reference implementations for running Cursor autonomously in loops. Covers everything from simple `claude -p` pipelines to full RFC-driven multi-agent DAG orchestration.
...
The `claude -p` flag runs Cursor non-interactively with a prompt, exits when done. Chain calls to build a pipeline:
...
1. Loads conversation history from `~/.cursor/claw/{session}.md`
...
### Implementation via Cursor Commands
...
**A production-grade shell script** that runs Cursor in a continuous loop, creating PRs, waiting for CI, and merging automatically. Created by AnandChowdhary (credit: @AnandChowdhary).
...
3. **Any loop + Verification** — Use ECC's `/ecc-verify` command command or `verification-loop` skill as a gate before commits.
