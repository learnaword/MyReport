...
# Config GC — Garbage Collection for Cursor Setups
...
- The user asks to clean up, audit, or slim down their Cursor configuration
...
Do NOT activate for: cleaning project source code (that's refactoring), clearing chat history, or uninstalling Cursor itself.
...
4. **Soft-delete first.** Rename to `.disabled` > move to `~/.cursor/_gc_trash/` > real deletion. Always keep an undo path.
5. **Forced human-in-the-loop.** Every candidate gets its own `[y/n/skip]` confirmation. No "yes to all" shortcut.
6. **Keep a log.** Every GC run appends to `~/.cursor/gc_log.md`: what was touched, why, and how to undo it.

## Scan Channels

| # | Channel | Path | Staleness / redundancy signals |
|---|---------|------|--------------------------------|
| 1 | Skills | `~/.cursor/skills/*/` | Heavily overlapping names; never triggered in recent transcripts; domain mismatch with the user's actual work; broken or empty SKILL.md |
| 2 | Memory | `~/.cursor/**/memory/*.md` + its index | Multiple index entries for one topic; contents contradicting newer entries; dates that have passed; orphan files missing from the index; sub-100-word fragments that should merge |
| 3 | Hooks | `~/.cursor/hooks/` + settings | Scripts present on disk but referenced by no hook config; old versions superseded by rewrites |
...
| 7 | Project history | `~/.cursor/projects/*/` | Stale handoff snapshots; session records superseded by newer state |
...
5. **Log** the run to `~/.cursor/gc_log.md`: timestamp, items actioned, undo instructions.
...
```bash
for f in ~/.cursor/hooks/*; do
  name=$(basename "$f")
  grep -rq "$name" ~/.cursor/settings.json ~/.cursor/settings.local.json 2>/dev/null \
    || echo "ORPHAN: $f"
done
```
...
```bash
jq -r '.permissions.allow[]' ~/.cursor/settings.local.json | sort | uniq -d
if jq -e '.permissions.allow | index("Bash(*)")' ~/.cursor/settings.local.json >/dev/null; then
  jq -r '.permissions.allow[]' ~/.cursor/settings.local.json \
...
```bash
find ~/.cursor/file-history ~/.cursor/shell-snapshots -type f -mtime +30 \
...
```bash
gc_date=$(date +%Y-%m-%d)
mkdir -p ~/.cursor/_gc_trash/$gc_date
mv ~/.cursor/skills/dead-skill ~/.cursor/_gc_trash/$gc_date/
echo "$(date -Iseconds) moved skills/dead-skill -> _gc_trash/$gc_date/ (undo: mv back)" >> ~/.cursor/gc_log.md
```

Removing a confirmed-redundant permission entry (JSON has no comments — back up, log, then edit):

```bash
cp ~/.cursor/settings.local.json ~/.cursor/settings.local.json.bak
echo "$(date -Iseconds) removed permission entry: Bash(git push) (undo: restore from .bak or re-add)" >> ~/.cursor/gc_log.md
jq '.permissions.allow -= ["Bash(git push)"]' ~/.cursor/settings.local.json.bak \
