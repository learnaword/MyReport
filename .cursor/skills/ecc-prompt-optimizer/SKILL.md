...
1. Check if a `project configuration` exists in the working directory — read it for project conventions
...
| HIGH | Cross-domain, 5+ files | /ecc-plan first, then phased execution |
...
|--------|----------|--------|--------|
| New Feature | /ecc-plan, /ecc-tdd, /ecc-code-review, /verify | tdd-workflow, verification-loop | planner, tdd-guide, code-reviewer |
| Bug Fix | /ecc-tdd, /ecc-build-fix, /verify | tdd-workflow | tdd-guide, build-error-resolver |
| Refactor | /ecc-refactor-clean, /ecc-code-review, /verify | verification-loop | refactor-cleaner, code-reviewer |
| Research | /ecc-plan | search-first, iterative-retrieval | — |
| Testing | /ecc-tdd, /ecc-e2e, /test-coverage | tdd-workflow, e2e-testing | tdd-guide, e2e-runner |
| Review | /ecc-code-review | security-review | code-reviewer, security-reviewer |
| Documentation | /update-docs, /update-codemaps | — | doc-updater |
| Infrastructure | /ecc-plan, /verify | docker-patterns, deployment-patterns, database-migrations | architect |
| Design (MEDIUM-HIGH) | /ecc-plan | — | planner, architect |
...
For MEDIUM+ tasks, always start with /ecc-plan. For EPIC tasks, use blueprint skill.
...
- Prompt 1: Research + Plan (use search-first skill, then /ecc-plan)
- Prompt 2-N: Implement one phase per prompt (each ends with /verify)
- Final Prompt: Integration test + /ecc-code-review across all phases
...
|------|-----------|---------|
| Command | /ecc-plan | Plan architecture before coding |
...
|--------|--------------|
| New Feature | `/ecc-plan [feature]. /ecc-tdd to implement. /ecc-code-review. /verify.` |
| Bug Fix | `/ecc-tdd — write failing test for [bug]. Fix to green. /verify.` |
| Refactor | `/ecc-refactor-clean [scope]. /ecc-code-review. /verify.` |
| Research | `Use search-first skill for [topic]. /ecc-plan based on findings.` |
| Testing | `/ecc-tdd [module]. /ecc-e2e for critical flows. /test-coverage.` |
| Review | `/ecc-code-review. Then use security-reviewer agent.` |
...
- "Rewrite this prompt so Cursor uses the right commands"
- "帮我优化这个指令"
...
工作流：
1. /ecc-plan 先规划组件结构和认证流程，参考现有页面的模式
2. /ecc-tdd 测试先行：编写登录表单的单元测试和认证流程的集成测试
3. 实现登录页面和认证逻辑
4. /ecc-code-review 审查实现
...
Workflow:
1. /ecc-plan the endpoint structure, middleware chain, and validation logic
2. /ecc-tdd — write table-driven tests for success, validation failure, auth failure, not-found
